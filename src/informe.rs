...
pub fn markdown(acta: &Acta) -> String {
    ...
    match &acta.sello_tiempo {
        Some(t) => {
            ...
            m.push_str(">\n");
            m.push_str("> **No hizo validación PKI completa** —sin revocación, sin restricciones\n");
            m.push_str("> de nombre, sin políticas—, y si el acta se verificó sin `--tsa-ca`,\n");
            m.push_str("> **la identidad de la autoridad no está acreditada**: un certificado\n");
            m.push_str("> autofirmado con ese mismo uso pasaría la comprobación criptográfica.\n");
            m.push_str("> Ambas cosas se hacen con la herramienta estándar:\n");
            m.push_str(">\n");
            m.push_str("> ```bash\n");
